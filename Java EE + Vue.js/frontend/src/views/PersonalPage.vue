<template>
  <div class="container personal-page">
    ...
    <button class="btn" @click="deleteAllPoints">Clear points</button>
  </div>
...
export default {
  ...
  methods: {
    ...
    async submitFormPoint() {
      if (this.point.x < -7 || this.point.y > 5 || this.point.y < -5 || this.point.x > 5 ||
          this.point.r < 1 || this.point.r > 6) {
    ...
    async deleteAllPoints() {
      try {
        if (this.points.length == 0) {
          this.showNotification("You don't have points, bro");
        } else {
          const authToken = localStorage.getItem('authToken');
          await api.clearPoints(authToken);
          this.showNotification('All points have been successfully deleted.');
          this.points = []; 
          await this.fetchPoints();
        }
      } catch (error) {
